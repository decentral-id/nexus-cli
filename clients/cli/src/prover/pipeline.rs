...
impl ProvingPipeline {
    ...
    pub async fn prove_authenticated(
        ...
        if total_memory_gb <= 2.0 {
            ...
            if let Ok(memory_usage) = std::fs::read_to_string("/proc/self/status") {
                if let Some(vmrss_line) =
                    memory_usage.lines().find(|line| line.starts_with("VmRSS:"))
                {
                    if let Some(mb_str) = vmrss_line.split_whitespace().nth(1) {
                        if let Ok(memory_kb) = mb_str.parse::<usize>() {
                            let memory_mb = memory_kb / 1024;
                            let safety_threshold_mb =
                                if total_memory_gb <= 1.5 { 800 } else { 1200 }; // Stricter for very low memory
                            ...
                            let estimated_memory_per_proof_mb = if total_memory_gb <= 0.95 {
                                180 // Back to working values - 2GB system worked with much less
                                ...
                                300 // Moderate for 2GB systems (was working)
                            } else {
                                400 // Standard for larger systems
                            };
                            let estimated_peak_memory_mb =
                                memory_mb + estimated_memory_per_proof_mb;

                            // Adaptive memory thresholds based on system size
                            let memory_threshold_factor = if total_memory_gb <= 0.95 {
                                0.55
                            } else if total_memory_gb <= 1.0 {
                                0.60
                            } else if total_memory_gb <= 1.5 {
                            ...
                            if unsafe { !CAPABILITIES_SHOWN } {
                                println!(
                                    "[SYSTEM] Sequential processing mode: can handle any number of inputs, one proof at a time"
                                );
                                println!(
                                    "[SYSTEM] Memory per proof: ~{} MB, system limit: {} MB",
                                    estimated_memory_per_proof_mb,
                                    (total_memory_gb * 1024.0 * 0.95) as usize
                                );
                                unsafe {
                            ...
                            println!(
                                "[MEMORY] Pre-task check passed: {} MB used, {} inputs will be processed sequentially (estimated ~{} MB per proof)",
                                memory_mb,
                                all_inputs.len(),
                                estimated_memory_per_proof_mb
                            );
    ...
    /// Process fibonacci proving task with multiple inputs (parallel implementation)
    async fn prove_fib_task_fully_isolated(
        ...
        // Parallel execution using persistent pool
        let concurrency = num_workers.max(1);

        // Use stream for parallelism
        // Clone inputs to ensure they are owned (Vec<u8>) for Send futures
        let inputs_with_indices: Vec<(usize, Vec<u8>)> = all_inputs
            .iter()
            .enumerate()
            .map(|(i, v)| (i, v.clone()))
            .collect();

        let results_with_indices: Vec<Result<(usize, Proof, String), ProverError>> =
            stream::iter(inputs_with_indices)
                .map(|(index, input_data)| {
                    async move {
        ...
        for res in results_with_indices {
            success_results.push(res?);
        }

        // Sort by index to restore original order
        success_results.sort_by_key(|(index, _, _)| *index);

        // Extract proofs and hashes using into_iter to move (avoid clone)
        let mut all_proofs = Vec::with_capacity(success_results.len());
        let mut proof_hashes = Vec::with_capacity(success_results.len());

        for (_, proof, hash) in success_results {
